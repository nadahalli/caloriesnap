...
An Android app that estimates calories and macros from a photo of your food, powered by on-device AI (Gemini Nano via ML Kit).

## How it works

1. Snap a photo of your meal
2. The on-device model identifies the food and estimates calories, protein, carbs, and fat
...
data/
  ...
  CalorieEstimator   # ML Kit implementation of FoodAnalyzer
...
Tests cover `EstimateParser` (prompt construction, JSON parsing, malformed input) and `MainViewModel` (download state machine, capture/analyze/save flow, error handling).

## Requirements

- Android 12+ (API 31)
- Device with camera
- Gemini Nano model downloaded on first launch
